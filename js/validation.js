...
Validation = (function($){
    var that = this;
    var options = $.extend(true, {}, Validation);
    var elemsCache = {
        body: $('body')
    };
    var Val = {
        init: function(){
            ...
            $.fn.validate = function(){
                return that.validate($(this));
            };
            $.fn.setupValidation = function(){
                return that.setupValidationOnForm($(this));
            };
            
            // ----

            $(window).bind('resize scroll', function(event){
                that.positionAllMessages();
            });
            ...
            elemsCache.body.on('click', '.validation-message', function(event){
                var msgElem = $(this);
                var targetElem = msgElem.data('target');
                that.removeMessage(targetElem, msgElem);
                targetElem.focus();
            });
        ...
        setupValidationOnForm: function(form){
            ...
            form.on('reset', function(){
                options.onResetForm.apply(form, [self]);
            });
        ...
        displayMessage: function(msg, targetElem){
            ...
            elem.appendTo(elemsCache.body);
            self.positionMessage(elem, targetElem);
            elem.fadeTo(options.fadeTime, 1);
        },
        ...
        positionMessage: function(msgElem, targetElem){
            ...
            var position = {
                left: offset.left + options.message.offset.x + targetElem.width(),
                top: offset.top + options.message.offset.y + (targetElem.outerHeight() - msgElem.outerHeight())/2
            };
            msgElem.css({
                top: position.top,
                left: position.left
        ...
        validateElement: function(element, event){
            ...
            var validationData = options.validationData.apply(element);
            ...
            (isElemValid ? options.onValid : options.onInvalid).apply(element);
